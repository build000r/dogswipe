...
- Production SPAPS auth proof is blocked on the final origin contract: this workspace has ignored private SPAPS values that pass release-readiness, and the backend is configured for SPAPS auth on the production host. `dogswipe.buildooor.com` is now live through a Cloudflare Worker custom domain with public health and AASA proof, but the DogSwipe SPAPS app still lacks `https://dogswipe.buildooor.com` in `allowed_origins`. No supported SPAPS self-service route for editing an existing app's origins is present in the current Sweet Potato API, so that remains an operator action rather than a public-repo change.
