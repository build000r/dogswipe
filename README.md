...
Current live-release probes show that the DogSwipe SPAPS app is registered and can be read with the ignored local handoff key, but its publishable-key origins must match the final production domain. If the release domain is `dogswipe.buildooor.com`, add that origin to SPAPS before shipping native auth. Local Xcode can create a development-signed archive for the DogSwipe bundle, but App Store/TestFlight export still needs an Apple Distribution certificate/profile through Xcode or App Store Connect API credentials.
...
- Live deployment and hosted universal-link activation are blocked until the production origin resolves publicly, the private env source is rendered on the host, the reverse proxy is wired to the DogSwipe API and AASA payload, and Apple Team ID values are available for the signed associated-domain build.
- Production SPAPS auth proof is blocked on live infrastructure, not the public app contract: this workspace has ignored private SPAPS values that pass release-readiness, and the repo can render the private env/overlay handoff. The production host now has the non-secret DogSwipe deploy layout and artifacts installed under `/opt/dogswipe`, but still needs `/opt/envs/dogswipe/prod.env`, reverse-proxy activation, and public DNS before hosted auth and universal links can be proven. Earlier private handoff used `dogswipe.build000r.com`; fresh DNS checks show `build000r.com` has no public NS/SOA response, while `buildooor.com` is active but has no `dogswipe` A record. The discovered Cloudflare credentials can either fail auth or read the active zone without DNS edit permission, so `dogswipe.buildooor.com` still needs a DNS-edit-capable token or a manual `A` record to `104.131.188.214`. If that becomes canonical, the SPAPS app allowed origins also need `https://dogswipe.buildooor.com`.
