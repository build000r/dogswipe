...
Production deploy artifacts live in [deploy/README.md](deploy/README.md). The repo now includes a production Compose file, conditional GHCR image publishing for backend-image changes on `main`, private env and skillbox overlay renderers, pre/post deploy verification scripts, a non-secret SPAPS app descriptor, an AASA render/readiness target, and a reverse-proxy site template. A live rollout still requires DNS, host setup, a private env source, Apple Team ID, health URL, and AASA URL.
