...
`spaps-registration-payload` renders the non-secret body for `POST /api/self-service/applications`; `spaps-origin-handoff` renders the no-secret operator SQL needed to add the current HTTPS release origin to an existing DogSwipe SPAPS app; the full operator flow is in [docs/SPAPS_APP_HANDOFF.md](docs/SPAPS_APP_HANDOFF.md). `deploy-render-aasa` renders the Apple app-site association payload with the same Apple Team ID and bundle identifier used by the signed archive. `edge-dry-run`, `edge-deploy`, and `edge-verify` manage the Cloudflare Worker custom-domain frontdoor for `dogswipe.buildooor.com`. `deploy-release-readiness` checks the live overlay, production release URLs, SPAPS registration payload, SPAPS publishable-key shape, AASA render, iOS release assets, optional DNS preflight, and optional App Store Connect API key handoff without printing secrets. `ios-release-archive` also accepts `DOGSWIPE_RELEASE_SPAPS_API_BASE_URL`, `DOGSWIPE_RELEASE_SPAPS_ORIGIN`, `DOGSWIPE_RELEASE_AUTH_REDIRECT_URL`, and `DOGSWIPE_RELEASE_AUTH_UNIVERSAL_LINK_HOSTS`; the latter three default from `DOGSWIPE_RELEASE_ASSOCIATED_DOMAIN`. `ios-testflight-upload` can upload the archive through an App Store Connect API key when `ASC_KEY_PATH`, `ASC_KEY_ID`, and `ASC_ISSUER_ID` are set. `.gitignore` blocks common Apple signing artifacts such as `.p8`, `.p12`, `.mobileprovision` files, and local Wrangler output.

Current live-release probes show that the DogSwipe SPAPS app is registered and now allows `https://dogswipe.buildooor.com` after the 2026-05-07 operator origin update. Keep the SPAPS release origin, universal-link host, Worker hostname, and iOS release associated domain aligned if the production host changes. Local Xcode can create a signed archive and App Store export for the DogSwipe bundle, but there is still no confirmed TestFlight upload because App Store Connect API issuer/upload access is not fully proven in this workspace.
...
Production deploy artifacts live in [deploy/README.md](deploy/README.md). The repo now includes a production Compose file, conditional GHCR image publishing for backend-image changes on `main`, private env and skillbox overlay renderers, a host bootstrap script for non-secret deploy files/directories, pre/post deploy verification scripts, a non-secret SPAPS app descriptor, an AASA render/readiness target, a reverse-proxy site template, and a Cloudflare Worker edge proxy. The production host has the private env rendered, Alembic migrations applied, the DogSwipe API running healthy on the current GHCR full-SHA image, public `https://dogswipe.buildooor.com/health` plus AASA proof through the Worker custom domain, and SPAPS allowed-origin alignment for the same HTTPS host. Public rollout still requires a confirmed App Store Connect/TestFlight upload.
...
- Edge verify: Cloudflare Worker public health and AASA URLs respond over the production hostname
...
GitHub Actions enforces the same blocking gates for backend coverage, CRAP, MMDX architecture syntax, SPAPS app contract and registration-payload validation, SwiftUI drift, deploy preflight, Worker edge dry-run, iOS release asset verification, iOS build/tests, and screenshot UI smoke.
...
- Production SPAPS auth origin alignment is live for `https://dogswipe.buildooor.com`, but full native auth proof still needs a signed TestFlight/App Store build exercising the universal-link return on device.
- Live TestFlight submission is blocked at upload confirmation: local Xcode can sign and export a DogSwipe archive, and an App Store Connect API key file is present outside the repo, but the API issuer/upload path still needs to be recovered or completed through Apple/Xcode.
