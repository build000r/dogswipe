...
- Production identity is backend-owned through SPAPS; the public `spaps.app.json` descriptor fixes the application slug to `dogswipe` without storing keys, the app may use a publishable key for native magic-link auth, handle `dogswipe://auth` and configured HTTPS universal-link returns, store access/refresh JWTs in Keychain, and send only user bearer tokens to the DogSwipe API.
...
- CI blocks regressions in backend coverage, scoped CRAP score, SwiftUI drift, architecture-diagram preflight, SPAPS app contract, deploy preflight/readiness, iOS release assets, iOS build/test gates, and screenshot UI smoke.
