...
| SPAPS live app metadata probe | ignored DogSwipe SPAPS handoff key against `/api/admin/apps` | app registration is readable and the slug is `dogswipe`; allowed origins still need the final HTTPS production host if the canonical domain is `dogswipe.buildooor.com` |
