...
| SPAPS live app metadata probe | read-only production SPAPS DB/container inspection | app registration exists with slug `dogswipe`; allowed origins still need the final HTTPS production host if the canonical domain is `dogswipe.buildooor.com` |
