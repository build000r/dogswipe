...
| SPAPS live app metadata probe | read-only production SPAPS DB/container inspection plus current Sweet Potato route inspection | app registration exists with slug `dogswipe`; allowed origins still need the final HTTPS production host if the canonical domain is `dogswipe.buildooor.com`; the current supported self-service surface has create/list/get/rotate/delete but no edit-origin endpoint |
...
- The ignored local SPAPS handoff file contains non-placeholder private app values in this workspace, the production host has its private env rendered, and a private release-readiness probe validates the app-side release contract; the remaining production auth gap is SPAPS allowed-origin alignment for the now-live `https://dogswipe.buildooor.com` origin.
...
- Production SPAPS auth proof remains blocked until the existing `dogswipe` SPAPS app includes `https://dogswipe.buildooor.com` in `allowed_origins`. The current Sweet Potato self-service surface does not expose an origin-edit route for the existing app. The DogSwipe API and public Worker edge are healthy, and hosted AASA proof passes, but native auth should not be called complete until the operator-side SPAPS origin update is applied.
