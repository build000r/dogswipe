...
| CI quality enforcement | GitHub Actions `25419200055` `backend`, `swift-package`, and `ios` jobs for executable-code commit `1077c93` | coverage XML feeds blocking CRAP; MMDX, SwiftUI drift, deploy/AASA render, Swift package, and iOS gates fail on regressions; iOS prefers a modern simulator, preboots it, disables parallel test workers, uses bounded destination/job/test timeouts, and runs direct-tab screenshot UI smoke |
