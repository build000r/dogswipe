...
| Production host/DNS probe | deploy/`ssh-info` read-only status, host bootstrap apply, host-side deploy preflight, migrations, internal health, and DNS/health checks for `dogswipe.build000r.com` and `dogswipe.buildooor.com` | SPAPS host reachable and healthy over legacy key-backed SSH; private DogSwipe env is installed under `/opt/envs/dogswipe/prod.env`; Alembic migrated through `0009`; Postgres, Redis, and the API are running healthy internally on `ghcr.io/build000r/dogswipe:ba9df2bf382fb24597d916e2212ce8522392a016`; unauthenticated discovery returns `401` as expected with SPAPS auth enabled; `build000r.com` has no public NS/SOA and `buildooor.com` is active on Cloudflare but has no `dogswipe` record |
...
- Live deployment, production SPAPS auth proof, and hosted universal-link activation are blocked until the canonical domain is real, the SPAPS allowed origins include that domain, and the shared reverse proxy serves the API plus Apple app-site association payload. A fresh 2026-05-06 probe shows `build000r.com` has no public NS/SOA, `dogswipe.build000r.com` is unresolved, `buildooor.com` is active on Cloudflare without a DogSwipe subdomain, and the discovered Cloudflare credentials cannot edit DNS. The DogSwipe API is running healthy internally with the private env installed, but the proxy site, AASA payload, certificates, DNS record, and public URL proof are still pending.
