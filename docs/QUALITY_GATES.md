...
- The deterministic Discover and Matches fixtures now use the supplied DogSwipe reference direction: cream/red/mustard street-vendor chrome, Chicago Classic hero cards, a draggable "Swipe right for hotdogs" control deck, and a match/order detail.
...
- Production deploy artifacts define the Compose stack, env contract, host bootstrap, DNS preflight, live-readiness wrapper, release-readiness checks, post-deploy verification, reverse-proxy template, bundle-aware Apple app-site association render path, and CI workflow.
- CI publishes a deterministic backend image tag to `ghcr.io/build000r/dogswipe:<full-git-sha>` plus `latest` for `main` pushes that change `backend/**` or `.github/workflows/ci.yml`; a recent verified tag is `ghcr.io/build000r/dogswipe:c00a5258ed08944691c0843e2830d773e663eb5d`.
...
- GitHub Actions enforces backend coverage XML generation, scoped CRAP threshold, MMDX architecture preflight, SPAPS app contract and registration-payload validation, SwiftUI drift, deploy preflight/readiness including host bootstrap, DNS preflight template, live-readiness template, and AASA template/render validation, iOS build/test gates, and screenshot UI smoke.
