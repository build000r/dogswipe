...
| Release readiness | `make deploy-release-readiness ALLOW_PLACEHOLDERS=true ...`; live-host template recheck with `CHECK_DNS=true DOGSWIPE_RELEASE_ASSOCIATED_DOMAIN=dogswipe.buildooor.com` | base placeholder run passed with 21 passed, 2 skipped, 0 failed; live-host DNS-enabled recheck passed with 22 passed, 1 skipped, 0 failed |
...
| iOS signing/TestFlight probe | `xcodebuild archive` with automatic signing; `xcodebuild -exportArchive` with App Store Connect export options | local signing/export can produce a DogSwipe archive/IPA, and Apple Distribution plus an App Store Connect API key file are present outside the repo; TestFlight remains unconfirmed until the App Store Connect issuer/upload path is completed |
...
- Full native SPAPS auth proof still needs a signed TestFlight/App Store build exercising the universal-link return on device; the production API, Worker edge, hosted AASA, and SPAPS origin metadata are aligned.
- Live App Store/TestFlight upload remains blocked until App Store Connect API issuer/upload access or an interactive Xcode Organizer upload path is completed.
