...
- GitHub Actions `25442723419`: `backend` succeeded in 2m14s including coverage, CRAP, MMDX, SPAPS app contract, SwiftUI drift, deploy preflight, AASA render smoke, private deploy handoff template, host bootstrap template, DNS preflight template, live-readiness template, release readiness, Docker image build, GHCR login, and GHCR publish; `swift-package` succeeded in 30s; and `ios` succeeded in 10m19s, including release asset verification, generic iOS build, unit tests, and screenshot UI smoke.
