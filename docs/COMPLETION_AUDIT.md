...
| Deploy preflight passes | CI run `25480313906` passed `make deploy-preflight`; the production host also passed preflight against the rendered private env with 19 passed, 0 warnings, and 0 failed. | Done |
...
| Host bootstrap template passes | CI run `25480313906` passed `make deploy-host-bootstrap-template`, which installs non-secret deploy artifacts into a temporary host layout and verifies deploy/env/data paths without writing secrets. | Done |
...
- CI `Coverage` gate in run `25480313906`: backend coverage passed the required 80.0% threshold.
